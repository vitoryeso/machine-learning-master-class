...
"Os números aqui mudam de leitura quando eu saio de uma seed só e faço o teste pareado com 30 seeds. A CNN 2D RGB fica com F1 em torno de 0,88; a versão em escala de cinza fica bem perto, em 0,86 — e a surpresa é que essa diferença **é ruído**, é empate estatístico: nessa tarefa de screenshot contra foto, **o canal de cor não agrega informação de verdade**. Já a CNN 1D, no perfil de linha, cai pra cerca de 0,83 — e essa diferença, sim, **é real e forte**: colapsar a imagem num vetor **joga fora a estrutura espacial** que a convolução 2D explora, e o teste pareado confirma isso com folga. Na ablação à direita: o número de filtros tem um ponto ótimo em 16, e o tamanho do kernel também — kernel 3 é o melhor, e kernel 7 despenca, porque um filtro grande demais numa imagem de 64 por 64 perde a localidade."
...
---
*4 slides · ~5:00. Se estourar, encurte o slide 3 lendo só "2D 0.88, 1D 0.83 (real), RGB×cinza empatam, k=3 melhor".*
