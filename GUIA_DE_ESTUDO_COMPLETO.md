...
### O que é "aprender"?
Temos exemplos `(x, y)`: **x** é a *entrada* (um vetor de números que descreve algo — chamado **features**), e **y** é o *alvo* (o que queremos prever). Aprender = achar uma função `f` tal que `f(x) ≈ y` em exemplos **que o modelo nunca viu, de outra distribuição** — formalmente, minimizando a **função de perda** (o erro médio nos exemplos):
...
A receita universal pra "ajustar parâmetros". Definimos a **função de perda** `L(θ)` (o erro médio do modelo, em função dos parâmetros `θ`). O **gradiente** `∇L` aponta na direção de maior *aumento* da perda; então andamos no sentido oposto:

```
L(θ) = (1/N) Σᵢ ℓ(f(xᵢ), yᵢ)
θ    ← θ − α · ∇L(θ)
```
